...
class Window(QWidget):
    ...
    ###加个功能，检测folder是否存在docx格式。如果存在的话，就弹出一个QMessageBox，报告完成。请查看。
            ...
            if "HomelessVideo" in varlist:
                ...
                for line in homelessvideo:
                    if "^\nSyntax Error" in line:
                        print("It is not supported in the system")
                        w.writerow(["It is not supported in the system"])
                    else:
                        if bool1:
